import type { ChartSpec, EChartsOption, Filter, GenerateRequest, GenerateResult, Outbound, Row, SentRequest } from './types'
import { summarizeData } from './data'
import { CHART_SPEC_SCHEMA, COUNT_DECISION_SCHEMA, COUNT_SYSTEM, SYSTEM_PROMPT, buildCountMessage, buildRepairMessage, buildSpecMessage } from './prompt'
...
async function claudeComplete(system: string, user: Outbound, key: string, schema: object = CHART_SPEC_SCHEMA): Promise<string> {
...
function enforceArgmaxFilter(spec: ChartSpec, prompt: string, rows: Row[]): ChartSpec {
...
function distinctValues(rows: Row[], col: string): string[] {
  const seen = new Set<string>()
  for (const r of rows) { const v = r[col]; if (v !== null && v !== undefined && v !== '') seen.add(String(v)) }
  return [...seen]
}

function enforceExclusion(spec: ChartSpec, prompt: string, rows: Row[]): ChartSpec {
  const cols = Object.keys(rows[0] ?? {})
  const dateCol = cols.find((c) => isDateColumn(rows, c))
  const added: Filter[] = []
  const wkVs = /\b(?:vs\.?|versus|compared (?:to|with)|comparison|against)\b/i.test(prompt)
  const keepWeekdays = !wkVs && (/\b(?:exclud\w+|skip\w*|without|except|sans|minus)\s+(?:the\s+)?weekends?\b/i.test(prompt) || /\bweekdays?\s+only\b/i.test(prompt) || /\b(?:only|just|restricted to|limited to|confined to|during|on)\s+(?:on\s+)?(?:the\s+)?weekdays?\b/i.test(prompt))
  const keepWeekends = !wkVs && (/\bweekends?\s+only\b/i.test(prompt) || /\b(?:only|just|restricted to|limited to|confined to|during|on)\s+(?:on\s+)?(?:the\s+)?weekends?\b/i.test(prompt))
  if (dateCol && keepWeekdays && !keepWeekends) added.push({ column: dateCol, datePart: 'weekday', in: [1, 2, 3, 4, 5] })
  else if (dateCol && keepWeekends && !keepWeekdays) added.push({ column: dateCol, datePart: 'weekday', in: [0, 6] })
  const exKw = prompt.match(/\b(?:except(?:\s+for)?|excluding|exclude|other than|all but|everything but|but not|not including|ignor\w+|non[-\s])/i)
  const catCols = cols.filter((c) => !isDateColumn(rows, c) && distinctValues(rows, c).some((v) => Number.isNaN(Number(v))))
  if (exKw) {
    const after = prompt.slice(exKw.index ?? 0, (exKw.index ?? 0) + 45).toLowerCase()
    for (const col of catCols) {
      const vals = distinctValues(rows, col)
      const excluded = vals.filter((v) => after.includes(v.toLowerCase()))
      if (excluded.length > 0 && excluded.length < vals.length) {
        added.push({ column: col, in: vals.filter((v) => !excluded.includes(v)) })
        break
      }
    }
  }
  if (added.length === 0) return spec
  return { ...spec, filters: [...(Array.isArray(spec.filters) ? spec.filters : []), ...added] }
}

function enforceSeries(spec: ChartSpec, prompt: string, rows: Row[]): ChartSpec {
  if (typeof spec.series === 'string' && spec.series.length > 0) return spec
  if (spec.chartType === 'pie' || spec.chartType === 'scatter') return spec
  const cols = Object.keys(rows[0] ?? {})
  const catCols = cols.filter((c) => !isDateColumn(rows, c) && distinctValues(rows, c).some((v) => Number.isNaN(Number(v))) && distinctValues(rows, c).length <= 12)
  if (catCols.length === 0) return spec
  const m = prompt.match(/\b(?:both|each|every|per|split(?:\s+(?:it|them))?\s+by|broken\s+down\s+by|grouped\s+by|one\s+(?:line|bar|series)\s+(?:per|for\s+each)|for\s+all)\s+(?:the\s+)?(\w+)/i)
  if (!m) return spec
  const col = matchColumn(m[1], catCols)
  if (!col || col === spec.x) return spec
  return { ...spec, series: col }
}
...
const SEASONS: Array<[RegExp, number[]]> = [[/\bsummer\b/i, [6, 7, 8]], [/\bwinter\b/i, [12, 1, 2]], [/\bspring\b/i, [3, 4, 5]], [/\b(?:fall|autumn)\b/i, [9, 10, 11]]]
...
function detectTimeWindow(prompt: string, rows: Row[]): Filter | null {
  ...
  const low = prompt.toLowerCase()
  for (const [re, months] of SEASONS) if (re.test(low)) return { column: dateCol, datePart: 'month', in: months }
  if (/\b(?:first|1st) half\b/.test(low)) return { column: dateCol, from: isoDay(year, 1, 1), to: isoDay(year, 6, 30) }
  if (/\b(?:second|2nd|back|latter) half\b/.test(low)) return { column: dateCol, from: isoDay(year, 7, 1), to: isoDay(year, 12, 31) }
  ...
  for (let i = 0; i < MONTH_NAMES.length; i++) if (new RegExp(`\\b${MONTH_NAMES[i]}\\b`, 'i').test(low)) monthsFound.push(i + 1)
  if (monthsFound.length >= 2) return { column: dateCol, datePart: 'month', in: monthsFound }
  const qs = [...new Set([...low.matchAll(/\bq([1-4])\b/g)].map((x) => Number(x[1])))]
  if (qs.length >= 2) return { column: dateCol, datePart: 'quarter', in: qs }
  return null
}

function enforceDateFilters(spec: ChartSpec, prompt: string, rows: Row[]): ChartSpec {
  const cols = Object.keys(rows[0] ?? {})
  ...
  if (xIsDate) {
    ...
    if (!spec.bucket && !spec.groupByPart) {
      if (/\b(?:by|per|each|across|through)\s+(?:the\s+)?week\s?days?\b|\bdays? of (?:the )?week\b|\b(?:across|through)\s+(?:the\s+)?week\b|\bweekday breakdown\b|\bbreakdown by weekday\b|\b(?:busiest|quietest|top|bottom|highest|lowest|best|worst)\s+\d*\s*week\s?days?\b|\b\d+\s+week\s?days?\s+with\b|\bweekends?\s+(?:vs\.?|versus|and|or|compared (?:to|with)|against)\s+weekdays?\b|\bweekdays?\s+(?:vs\.?|versus|and|or|compared (?:to|with)|against)\s+weekends?\b/i.test(prompt)) spec = { ...spec, groupByPart: 'weekday' }
      else if (/\bday of (?:the )?month\b/i.test(prompt)) spec = { ...spec, groupByPart: 'day' }
      else if (/\b(?:by|per|across|each) month of (?:the )?year\b/i.test(prompt)) spec = { ...spec, groupByPart: 'month' }
    }
    if (!spec.bucket && !spec.groupByPart) {
...
function buildFrom(raw: string, prompt: string, rows: Row[], toReal: Record<string, string>, allowEmpty = false): { option: EChartsOption } | { error: string } {
  ...
  try {
    option = buildChartOption(enforceSeries(enforceExclusion(enforceArgmaxFilter(enforceDateFilters(detokenizeSpec(parsed as ChartSpec, toReal), prompt, rows), prompt, rows), prompt, rows), prompt, rows), rows, allowEmpty)
  } catch (e) {
...
async function refineCount(raw: string, safePrompt: string, key: string, sent: SentRequest[]): Promise<string> {
  let spec: ChartSpec
  try { spec = JSON.parse(extractJson(raw)) as ChartSpec } catch { return raw }
  if (typeof spec !== 'object' || spec === null) return raw
  if ((spec.aggregate ?? 'sum') !== 'sum') return raw
  if (spec.derived || (Array.isArray(spec.measures) && spec.measures.length > 0)) return raw
  if (spec.chartType === 'pie' || spec.chartType === 'scatter') return raw
  if (typeof spec.measure !== 'string' || typeof spec.x !== 'string') return raw
  const conds = [...(Array.isArray(spec.filters) ? spec.filters : []), ...(spec.filter ? [spec.filter] : [])]
    .filter((f): f is Filter => !!f && !!f.op)
    .map((f) => `${f.column} ${f.op} ${f.value ?? (Array.isArray(f.in) ? f.in[0] : '')}`)
  if (conds.length === 0) return raw
  try {
    const msg = buildCountMessage(safePrompt, spec.measure, spec.x, conds.join(' and '))
    sent.push({ system: COUNT_SYSTEM, user: msg })
    const out = await claudeComplete(COUNT_SYSTEM, msg, key, COUNT_DECISION_SCHEMA)
    const decision = (JSON.parse(extractJson(out)) as { decision?: string }).decision
    if (decision === 'count') return JSON.stringify({ ...spec, aggregate: 'count' })
  } catch { return raw }
  return raw
}
...
export async function generateOption(req: GenerateRequest): Promise<GenerateResult> {
  ...
  const { message, toReal, safePrompt } = deidentify(req.prompt, req.rows)

  let raw: string
  try {
    sent.push({ system: SYSTEM_PROMPT, user: message })
    raw = await claudeComplete(SYSTEM_PROMPT, message, key)
    raw = await refineCount(raw, safePrompt, key, sent)
  } catch (e) {
  ...
  return { status: 'failed', error: second.error, raw: raw2, sent }
}
